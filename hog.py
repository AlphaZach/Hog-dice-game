...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        ...
        if is_swap(score0, score1):
            tem_score = score0
            score0, score1 = score1, tem_score
        player = other(player)
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    # BEGIN PROBLEM 7
    def highest_gain(score0, score1):
        high, pre_score = previous_high, previous_score
        #for player 1
        if who == 1:
            if score1 - pre_score > high:
                high = score1 - pre_score
                if high == 1:
        ...
        #for player 0
        if who == 0:
            if score0 - pre_score > high:
                high = score0 - pre_score
                if high == 1:
                    print(high , "point! That's the biggest gain yet for Player" , who)
                else: 
                    print(high , "points! That's the biggest gain yet for Player" , who)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    counter, max_avg, num_roll = 1, 0, 1
    while counter <= 10:
        avg = make_averaged(roll_dice)(counter, dice)
        if avg > max_avg:
            max_avg, num_roll = avg, counter
        counter += 1
    return num_roll
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    roll_0 = score + free_bacon(opponent_score)
    if free_bacon(opponent_score) >= margin:
        return 0
    if is_swap(roll_0, opponent_score) and roll_0 < opponent_score:
        return 0
    return num_rolls  # Replace this statement
    # END PROBLEM 11
